...
def _workflow_figure(split_counts: pd.DataFrame) -> plt.Figure:
    ...
    boxes = [
        ...
        (0.54, f"Calibration subjects\n{int(split_counts.loc['calibration', 'subjects'])}\nCalibration + raw-score conformal", COLORS["green"]),
        (0.80, f"Development holdout\n{int(split_counts.loc['test', 'subjects'])} subjects\nExploratory evaluation", COLORS["red"]),
    ]
...
def _aggregation_figure() -> plt.Figure:
    ...
    ax.annotate("", xy=(0.86, 0.5), xytext=(0.78, 0.5), xycoords="axes fraction",
                arrowprops={"arrowstyle": "->", "lw": 2})
    ax.text(0.93, 0.5, "Raw-score\nconformal set", ha="center", va="center", transform=ax.transAxes,
            bbox={"boxstyle": "round,pad=0.7", "facecolor": COLORS["green"], "edgecolor": "none"}, color="white")
    ax.text(0.5, 0.08, "A singleton yields a non-abstained model prediction; {healthy, PD} triggers abstention.",
            transform=ax.transAxes, ha="center", fontsize=10)
    return fig
...
def generate_uncertainty_paper_outputs(directory: Path, study: dict[str, object]) -> None:
    ...
    assets.append({"figure": 1, "files": files, "caption": "Subject-disjoint training, calibration, and development-holdout workflow."})
    ...
    test_subjects = predictions["subject_id"].nunique()
    ax1.set(xlabel="Mean predicted PD probability", ylabel="Observed PD frequency", title=f"Reliability diagram (n = {test_subjects} development subjects)")
    ax1.legend()
    ...
    assets.append({"figure": 4, "files": files, "caption": f"Reliability and calibration error for the selected model on {test_subjects} held-out development subjects."})
    ...
    ax.set(xlabel="Target coverage (1 - alpha)", ylabel="Empirical development-holdout coverage", xlim=(0.77, 0.97), ylim=(0.7, 1.02))
    ax.legend()
    files = _save(fig, directory, "figure_5_conformal_coverage")
    assets.append({"figure": 5, "files": files, "caption": "Empirical versus target subject-level conformal coverage on the held-out development partition, using raw subject probabilities."})
    ...
    panels = (
        ...
        ("coverage", "Conformal coverage (higher)"),
        ("abstention_rate", "Abstention rate"),
        ("selective_accuracy", "Selective accuracy (higher)"),
    )
    colors = [COLORS["blue"], COLORS["orange"], COLORS["green"]]
    labels = [_display(x) for x in aggregation["strategy"]]
    for ax, (metric, title) in zip(axes.flat, panels):
        values = aggregation[metric].to_numpy(dtype=float)
        low = aggregation[f"{metric}_ci_low"].to_numpy(dtype=float)
        high = aggregation[f"{metric}_ci_high"].to_numpy(dtype=float)
        error = np.maximum(0, np.vstack([values - low, high - values]))
        finite = np.isfinite(values) & np.isfinite(error).all(axis=0)
        ax.bar(
            labels,
            np.where(finite, values, 0),
            color=colors,
            yerr=np.where(finite[None, :], error, 0),
            capsize=4,
        )
        for index in np.flatnonzero(~finite):
            ax.text(
                index, 0.04, "Undefined\n(no singletons)",
    ...
    assets.append({"figure": 8, "files": files, "caption": "Comparison of calibrated classification and raw-score conformal metrics across repeated-recording aggregation strategies; bars show 95% subject-bootstrap intervals."})
    ...
    assets.append({"figure": 9, "files": files, "caption": "Class-conditional conformal coverage by sex and diagnosis with 95% Wilson intervals."})

    final_predictions = study["final_predictions"]
    forced = (final_predictions["platt_probability"] >= 0.5).astype(int)
    ...
    primary_alpha = float(study["primary_alpha"])
    assets.append({"figure": 10, "files": files, "caption": f"Platt-scaled forced classification versus raw-score class-conditional conformal singleton model predictions at alpha={primary_alpha:.2f}."})
    ...
    ax.set(ylabel="Recording-level score", ylim=(0, 1.05), title="Recording-level split sensitivity"); ax.legend()
    files = _save(fig, directory, "figure_11_leakage_sensitivity")
    assets.append({"figure": 11, "files": files, "caption": "Recording-level accuracy and ROC-AUC under both a random recording split (subject leakage possible) and a subject-disjoint split."})
